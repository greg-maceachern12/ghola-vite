...
const GeneratedImage = ({ src, alt, character }) => {
  ...
  const handleShare = async () => {
    ...
    try {
      ...
      await navigator.share({
        ...
        text: `Check out this AI-generated image of ${character || 'a character'} created with Ghola!`,
        files: [file]
      });
    } catch (error) {
    ...
    <div className="w-full">
      <div className="relative rounded-xl overflow-hidden transition-all duration-300 transform">
        ...
        <img
          src={src}
          alt={alt}
          className={`w-full h-auto rounded-xl object-cover max-h-[600px] transition-opacity duration-500 ${isLoaded ? 'opacity-100' : 'opacity-0'}`}
      ...
      {isLoaded && (
        <div className="flex justify-between items-center mt-4">
          <div className="text-xs text-white/50 flex items-center gap-1">
            <FaInfoCircle />
            <span>Generated with Flux Schnell model</span>
          </div>
